...
Open `CHANGELOG.md` and fill in the new version section that the script created. Replace the placeholder lines with actual release notes grouped under:
- `### Added` — new features
- `### Changed` — modifications to existing behavior
- `### Fixed` — bug fixes
- `### Removed` — anything deleted (omit if empty)
...
Parse the new version's section from `CHANGELOG.md` to use as the release body:
```bash
VERSION=$(node -p "require('./package.json').version")
# Print lines between the new version header and the next ## header
...
Confirm the draft appears at `https://github.com/GOODBOY008/r-shell/releases`.
