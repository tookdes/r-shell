...
describe('ConnectionStorageManager.buildConnectionTree ordering', () => {
  ...
  it('manually ordered items sort before auto-appended items (stable)', () => {
    // saveConnection auto-assigns increasing sortOrder (creation order); a
    // manual reorder must win over that auto order, and new connections are
    // appended after the manually reordered ones.
    const alpha = createConnection('Alpha', 'All Connections'); // auto sortOrder 0
    const beta = createConnection('Beta', 'All Connections'); // auto sortOrder 1

    // Simulate dragging Beta above Alpha (manual reorder)
    ConnectionStorageManager.reorderItem(beta.id, 'connection', 'All Connections', 0);

    const gamma = createConnection('Gamma', 'All Connections'); // auto sortOrder 2

    const tree = ConnectionStorageManager.buildConnectionTree();
    const root = tree[0];
    const connNames = root.children!.filter(c => c.type === 'connection').map(c => c.name);

    // Manual order (Beta, Alpha) wins; auto-appended Gamma stays at the end
    expect(connNames).toEqual(['Beta', 'Alpha', 'Gamma']);
  });
